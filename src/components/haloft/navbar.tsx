...
export function Navbar() {
  ...
  // A sentinel at the top of the page rather than a scroll listener: the
  // browser tells us when it leaves the viewport instead of us running
  // JavaScript on every scroll frame.
  useEffect(() => {
  ...
  return (
    <>
      <div ref={sentinelRef} aria-hidden="true" className="absolute top-6 h-px w-px" />
      ...
      >
        ...
        {open && (
          <div
            className="sheet-in fixed inset-0 z-[60] flex flex-col bg-[var(--paper)] lg:hidden"
            role="dialog"
            aria-modal="true"
            aria-label="Menu"
          >
            ...
            <nav className="shell flex flex-1 flex-col justify-center gap-1 pb-8" aria-label="Mobile">
              {NAV_LINKS.map((link, i) => (
                <Link
                  ...
                  href={link.href}
                  onClick={() => setOpen(false)}
                  className="sheet-item flex items-baseline justify-between border-b border-[var(--line)] py-5 text-[26px] font-medium tracking-[-0.03em] text-ink"
                  style={{ "--reveal-delay": `${50 + i * 50}ms` } as React.CSSProperties}
                >
                  {link.label}
