    ...
    <div className="flex min-h-full flex-col">
      ...
      <main className="flex-1">
          ...
          <div className="container mx-auto px-4 lg:px-8 pt-16 pb-14 lg:pt-24 lg:pb-20">
            <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-12 lg:gap-8 items-center">
              ...
              <div className="max-w-xl">
                ...
                {/* Working search — Airbnb style */}
                <form
                  action={`/${DEFAULT_CAMPUS_SLUG}/search`}
                  className="flex items-stretch gap-0 rounded-2xl border border-border bg-card shadow-lg shadow-black/5 p-1.5 mb-4 animate-slide-up stagger-2 focus-within:border-primary/40 transition-colors h-14"
                >
                  ...
                  <Button type="submit" className="rounded-xl px-6 font-semibold shrink-0 h-full">
                ...
                {/* Popular searches */}
                <div className="flex flex-wrap items-center gap-2 animate-fade-in stagger-3 mb-6">
                  <span className="text-xs text-muted-foreground font-medium mr-0.5">Popular searches:</span>
                  {["Self contain", "Hostel", "Near gate", "Wi-Fi included"].map((term) => (
                    <Link
                      key={term}
                      href={`/${DEFAULT_CAMPUS_SLUG}/search?q=${encodeURIComponent(term)}`}
              ...
              <div className="relative hidden lg:flex items-center justify-center animate-fade-in stagger-2" aria-hidden="true">
                ...
                <div className="absolute bottom-2 right-0 rounded-full border border-border bg-card px-4 py-2 shadow-lg shadow-black/5 animate-float" style={{ animationDelay: "0.6s" }}>
        ...
        <section className="bg-night text-night-foreground">
          <div className="container mx-auto px-4 lg:px-8 py-8">
            <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-4">
              {[
                { icon: ShieldCheck, text: "Every listing inspected in person" },
                { icon: BadgeCheck, text: "No agent fees — ever" },
                { icon: CreditCard, text: "Pay online with Paystack" },
                { icon: Home, text: "Rooms from ₦60k a year" },
              ].map((fact, i, arr) => (
                <div key={fact.text} className="flex items-center gap-6">
                  <div className="flex items-center gap-2.5">
                    <fact.icon className="h-5 w-5 text-night-foreground/70 shrink-0" />
                    <p className="text-sm font-medium text-night-foreground/85">{fact.text}</p>
                  </div>
                  {i < arr.length - 1 && (
                    <div className="hidden md:block h-4 w-px bg-night-foreground/20" />
                  )}
                </div>
              ))}
            </div>
                ...
                {steps.map((item) => (
                  <div key={item.n} className="relative">
                    <div className="relative">
                      ...
                      <span className="absolute -top-4 -left-2 heading-display text-[100px] font-extrabold text-foreground/[0.04] select-none pointer-events-none leading-none">
                        {item.n}
                      </span>
                      <div className="relative flex items-baseline gap-3 mb-3">
                        <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                          <span className="heading-display text-lg font-extrabold text-primary">{item.n}</span>
                        </div>
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed md:pr-4 relative">
                      {item.description}
                    </p>
                  </div>
                ))}
          ...
          <div className="container mx-auto px-4 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {proofs.map((p) => {
                const Icon = p.icon;
                return (
                <div key={p.title} className="rounded-2xl border border-border bg-card p-6 shadow-sm hover:shadow-lg hover:shadow-black/5 transition-shadow">
        ...
        <section className="relative overflow-hidden bg-night">
          <div className="pointer-events-none absolute inset-0 flex items-center justify-end opacity-[0.05]" aria-hidden="true">
          ...
          <div className="container mx-auto px-4 lg:px-8 section-py relative">
            <div className="max-w-2xl">
              ...
              <div className="flex flex-wrap gap-3 mb-8">
                <div className="rounded-2xl border border-night-foreground/10 bg-night-foreground/5 backdrop-blur-sm px-4 py-3">
                  <p className="text-night-foreground font-bold text-sm">₦60k min</p>
                  <p className="text-night-foreground/50 text-xs">Starting price</p>
                </div>
                <div className="rounded-2xl border border-night-foreground/10 bg-night-foreground/5 backdrop-blur-sm px-4 py-3">
                  <p className="text-night-foreground font-bold text-sm">5 Neighbourhoods</p>
                  <p className="text-night-foreground/50 text-xs">Around campus</p>
                </div>
                <div className="rounded-2xl border border-night-foreground/10 bg-night-foreground/5 backdrop-blur-sm px-4 py-3">
                  <p className="text-night-foreground font-bold text-sm">Verified listings</p>
                  <p className="text-night-foreground/50 text-xs">Inspected in person</p>
                </div>
              </div>
